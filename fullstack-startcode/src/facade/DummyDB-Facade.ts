...
class FriendsFacade {
  ...
  async addFriend(friend: IFriend): Promise<IFriend> {
    let newFriend: IFriend | null
    newFriend = this.friends.find(f => f.email === friend.email) ? null : friend
    console.log(newFriend)
    if(newFriend != null){
      newFriend.id = "id" + this.friends.length + 1
      this.friends.push(newFriend)
      return newFriend
    }else{
      throw new Error("Something went wrong, try again without knowing what to do different")
    }
  }
